...
class SeleniumOperator(object):
    ...
    def __init__(self, ip='192.168.1.1', username='admin', password='admin', port=80,
        ...
        print self.sel
        driver_dict = {ConfigConstant.BROWSER_IE:webdriver.Ie,
                       ConfigConstant.BROWSER_FIREFOX:webdriver.Firefox,
                       ConfigConstant.BROWSER_CHROME:webdriver.Chrome}
        if self.sel is None:
            if driver_dict.has_key(self.sel_browser):
                self.__class__.sel = driver_dict[self.sel_browser]()
            else:
                self.__class__.sel = webdriver.Remote("http://%s:%s/wd/hub"%(self.sel_host, self.sel_port), 
    ...
    def is_widget_present(self, widget):
        try:
            self.sel.find_element_by_xpath(widget.xpath)
            return True
        except:
            return False
    ...
    def set(self, widget=None, method=None, value=None, timeout=None):
        ...
        try:
            ...
            self.sel.switch_to_frame(widget.frame)
        except:
            pass
            
        widget_handler = self.get_element(widget)
        ...
        # if value is None:
            # exec("widget_handler.%s()"%widget.calls)
            
        # do method according to widget type    
        if widget.type == ConfigConstant.WIDGET_TYPE_TEXT_BOX:
            widget_handler.clear()
            widget_handler.send_keys(value)
        else:
            exec("widget_handler.%s()"%widget.calls)
    ...
    def get_location(self):
        return self.sel.current_url
        
    def get_element(self, widget):
        return self.sel.find_element_by_xpath(widget.xpath)
        
    def get_value(self, widget):
        element = self.get_element(widget)
        if element.text:
            return element.text
        else:
    ...
    def wait_for_page_to_load(self, timeout=None):
        if timeout is None:
            timeout = 150000
        self.sel.implicitly_wait(timeout)      
