...
logger = logging.getLogger('my_ping') 
...
def send_one_ping(my_socket, dest_addr, ID):
    ...
    header = struct.pack(
        "bbHHh", ICMP_ECHO_REQUEST, 0, socket.htons(my_checksum), ID, SEQUENCE_NUMBER
    )
    packet = header + data
...
def do_one(dest_addr, timeout):
    ...
    try:
        ...
        raise socket.error(errno, msg) # raise the original error
...
def verbose_ping(dest_addr, timeout = 2, count = 4):
    ...
    for i in xrange(count):
        ...
        try:
            ...
            logger.debug("failed. (socket gaierror: '%s: %s')" % (e[0], e[1]))
            break
...
def ping(dest_addr, timeout = 2, count = 4):
    ...
    for i in xrange(count):
        ...
        try:
            ...
            logger.debug("failed. (socket error: '%s: %s')" % (e[0], e[1]))
            continue
    ...
    logger.debug("Packets: Sent = %s, Received = %s, Lost = %s (%s%% loss)" % (count, response_num, loss, loss_rate))
